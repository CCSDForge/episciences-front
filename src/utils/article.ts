 ...
 */
export const extractAbstractText = (
...
export const formatArticle = (article: RawArticle): FetchedArticle => {
  if (article['@id']) {
    ...
    const articleContent =
      articleJournal?.journal_article ?? articleConference.conference_paper;
    ...
    if (articleContent.abstract?.value) {
      ...
      if (Array.isArray(value)) {
        ...
        // Case 1.3: Array of objects with value and @xml:lang properties
          ...
          const isAbstractItemCompatible = (
            item: unknown
          ): item is { '@xml:lang': string; value: string } => {
      ...
      // Case 2: A single object
        ...
        if (
          objectValue['@xml:lang'] !== undefined &&
    ...
    if (articleDB.current.cited_by) {
      citedBy = Object.values(articleDB.current.cited_by).map(cb => ({
        ...
        citations: Object.values(
          JSON.parse(cb.citation as unknown as string)
        ).map(c => {
    ...
    if (Array.isArray(articleContent.contributors.person_name)) {
      ...
      const sortedAuthors = articleContent.contributors.person_name.sort(
        (a, b) =>
          authorOrder[a['@sequence'] as keyof typeof authorOrder] -
          authorOrder[b['@sequence'] as keyof typeof authorOrder]
      );
      authors = sortedAuthors.map(author => {
        ...
        if (Array.isArray(author.affiliations?.institution)) {
          institutions = author.affiliations?.institution.map(
            i => i.institution_name
          );
        } else {
      ...
      if (
        Array.isArray(
          articleContent.contributors.person_name.affiliations?.institution
        )
      ) {
        authorInstitutions =
          articleContent.contributors.person_name.affiliations?.institution.map(
            i => i.institution_name
          );
      } else {
        if (
    ...
    if (Array.isArray(articleContent.program)) {
      articleContent.program.forEach(prog => {
        if (Array.isArray(prog?.related_item)) {
          prog.related_item.forEach(item => {
            if (item.inter_work_relation) {
              relatedItems.push({
                ...
                relationshipType:
                  item.inter_work_relation['@relationship-type'],
            ...
            if (item.intra_work_relation) {
              relatedItems.push({
                ...
                relationshipType:
                  item.intra_work_relation['@relationship-type'],
          ...
          relatedItems.push({
            ...
            identifierType:
              prog.related_item.inter_work_relation['@identifier-type'],
            relationshipType:
              prog.related_item.inter_work_relation['@relationship-type'],
            citation:
              prog.related_item.inter_work_relation.unstructured_citation,
          });
        } else if (prog?.related_item?.intra_work_relation) {
          relatedItems.push({
            value: prog.related_item.intra_work_relation?.value,
            identifierType:
              prog.related_item.intra_work_relation['@identifier-type'],
            relationshipType:
              prog.related_item.intra_work_relation['@relationship-type'],
            citation:
              prog.related_item.intra_work_relation.unstructured_citation,
          });
        }
      });
    } else {
      if (Array.isArray(articleContent.program?.related_item)) {
        ...
        relatedItems.push({
          ...
          identifierType:
            articleContent.program.related_item.inter_work_relation[
              '@identifier-type'
            ],
          relationshipType:
            articleContent.program.related_item.inter_work_relation[
        ...
        relatedItems.push({
          ...
          identifierType:
            articleContent.program.related_item.intra_work_relation[
              '@identifier-type'
            ],
          relationshipType:
            articleContent.program.related_item.intra_work_relation[
    ...
    if (Array.isArray(articleContent.program)) {
      const fundref = articleContent.program.find(
        p => p['@name'] && p['@name'] === 'fundref'
      );
      if (fundref) {
      ...
      if (
        ...
        articleContent.program['@name'] === 'fundref'
      ) {
        if (Array.isArray(articleContent.program.assertion?.assertion)) {
          articleContent.program.assertion?.assertion?.forEach(as =>
            fundings.push(as.value)
          );
    ...
    /** Format metrics */
    const metrics: { views: number; downloads: number } = {
      views: 0,
      downloads: 0,
    };
    ...
    return {
      ...
      pdfLink: articleDB.current.mainPdfUrl?.length
        ? articleDB.current.mainPdfUrl
        : void 0,
      // Add a check for null or undefined before accessing length
      docLink: articleDB.current.repository.doc_url?.length
        ? articleDB.current.repository.doc_url
        : void 0,
...
export const truncatedArticleAuthorsName = (
...
export const articleTypes: { labelPath: string; value: string }[] = [
  ...
  {
    labelPath: 'pages.articles.types.conference',
    value: ARTICLE_TYPE.CONFERENCE,
  },
...
export const citationCustomTemplates: {
...
export const getCitations = async (csl?: string): Promise<ICitation[]> => {
  ...
  await Promise.all(
    citationCustomTemplates.map(async customTemplate => {
  ...
  await Promise.all(
    Object.values(CITATION_TEMPLATE)
      .filter(template => template !== CITATION_TEMPLATE.BIBTEX)
      .map(async template => {
...
export const getLicenseTranslations = (
  ...
  },
  {
    ...
    label: t(
      'pages.articleDetails.licenses.creativeCommons.noDerivativesNonCommercial1.0'
    ),
    isLink: true,
  },
  {
    ...
    label: t(
      'pages.articleDetails.licenses.creativeCommons.noDerivativesNonCommercial2.0'
    ),
    isLink: true,
  },
  {
    ...
    label: t(
      'pages.articleDetails.licenses.creativeCommons.noDerivativesNonCommercial2.5'
    ),
    isLink: true,
  },
  {
    ...
    label: t(
      'pages.articleDetails.licenses.creativeCommons.noDerivativesNonCommercial3.0'
    ),
    isLink: true,
  },
  {
    ...
    label: t(
      'pages.articleDetails.licenses.creativeCommons.noDerivativesNonCommercial4.0'
    ),
    isLink: true,
  },
  {
    ...
    label: t(
      'pages.articleDetails.licenses.creativeCommons.nonCommercialShareAlike1.0'
    ),
    isLink: true,
  },
  {
    ...
    label: t(
      'pages.articleDetails.licenses.creativeCommons.nonCommercialShareAlike2.0'
    ),
    isLink: true,
  },
  {
    ...
    label: t(
      'pages.articleDetails.licenses.creativeCommons.nonCommercialShareAlike2.5'
    ),
    isLink: true,
  },
  {
    ...
    label: t(
      'pages.articleDetails.licenses.creativeCommons.nonCommercialShareAlike3.0'
    ),
    isLink: true,
  },
  {
    ...
    label: t(
      'pages.articleDetails.licenses.creativeCommons.nonCommercialShareAlike4.0'
    ),
    isLink: true,
  },
  {
...
export const getMetadataTypes: {
  ...
  {
    type: METADATA_TYPE.CROSSREF,
    label: 'Crossref',
    format: METADATA_FORMAT.XML,
  },
...
export const interworkRelationShipTypes: {
  labelPath: string;
  value: string;
}[] = [
  {
