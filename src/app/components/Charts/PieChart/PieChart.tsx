...
export default function PieChart({ t, data }: IPieChartProps): JSX.Element {
  ...
  const CHART_COLORS = [
    import.meta.env.VITE_JOURNAL_STATISTICS_COLORS_0 || '#9A312C',
    import.meta.env.VITE_JOURNAL_STATISTICS_COLORS_1 || '#C9605B',
    import.meta.env.VITE_JOURNAL_STATISTICS_COLORS_2 || '#FF9994',
    import.meta.env.VITE_JOURNAL_STATISTICS_COLORS_3 || '#FFC9C7',
  ];

  const getLegend = (): JSX.Element => {
    const notBeingToPublishStatuses = data.filter(
      singleData => !singleData.isBeingToPublishStatus
    );
    const beingToPublishStatuses = data.filter(
      singleData => singleData.isBeingToPublishStatus
    );
