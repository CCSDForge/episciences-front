...
export default function AcknowledgementsSidebar({
  headers,
  toggleHeaderCallback,
}: IAcknowledgementsSidebarProps): JSX.Element {
  return (
    <div className="acknowledgementsSidebar">
      {headers.map((header, index) => (
        <div key={index} className="acknowledgementsSidebar-header">
          <div className="acknowledgementsSidebar-header-title">
            <Link to={`#${header.id}`}>
              <div className="acknowledgementsSidebar-header-title-text">
                {header.value}
              </div>
            </Link>
            {header.children.length > 0 && (
              <img
                className="acknowledgementsSidebar-header-title-caret"
