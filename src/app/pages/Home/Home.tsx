...
export default function Home(): JSX.Element {
  ...
  const { data: aboutPage } = useFetchAboutPageQuery(rvcode!, {
    skip: !rvcode,
  });
  const { data: articles } = useFetchArticlesQuery(
    { rvcode: rvcode!, page: 1, itemsPerPage: 20 },
    { skip: !rvcode }
  );
  ...
  const { data: indexation } = useFetchIndexationPageQuery(rvcode!, {
    skip: !rvcode,
  });
  const { data: volumes } = useFetchVolumesQuery(
    { rvcode: rvcode!, language: language, page: 1, itemsPerPage: 2 },
    { skip: !rvcode }
  );
  const { data: issues } = useFetchVolumesQuery(
    {
      rvcode: rvcode!,
      language: language,
      page: 1,
  ...
  const { data: acceptedArticles } = useFetchArticlesQuery(
    { rvcode: rvcode!, page: 1, itemsPerPage: 20, onlyAccepted: true },
    { skip: !rvcode }
  );

  const getBlockRendering = (blockKey: HOMEPAGE_BLOCK) =>
    blocksConfiguration().find(config => config.key === blockKey);

  const getLastInformation = ():
    | {
        type: HOMEPAGE_LAST_INFORMATION_BLOCK;
        information: IVolume | INews | undefined;
      }
    | undefined => {
    if (!lastInformationBlockConfiguration().render) return;

    if (
      lastInformationBlockConfiguration().key ===
      HOMEPAGE_LAST_INFORMATION_BLOCK.LAST_VOLUME
    ) {
      return {
    ...
    if (
      lastInformationBlockConfiguration().key ===
      HOMEPAGE_LAST_INFORMATION_BLOCK.LAST_SPECIAL_ISSUE
    ) {
      return {
  ...
  return (
    <main className="home">
      <Helmet>
        <title>
          {t('pages.home.title')} | {journalName ?? ''}
        </title>
      </Helmet>
      ...
      {getBlockRendering(HOMEPAGE_BLOCK.LATEST_ARTICLES_CAROUSEL)?.render && (
        <>
            ...
            <Link to={PATHS.articles}>
              <div className="home-subtitle-all">
                <div className="home-subtitle-all-text">
                  {t('pages.home.blocks.articles.see')}
                </div>
          ...
          <Swiper
            ...
            slidesPerView={3}
            slidesPerGroup={3}
            cards={articles?.data.filter(article => article?.title) ?? []}
          />
        </>
      )}
        ...
        <>
          <div className="home-subtitle">
            ...
            <Link to={PATHS.boards}>
              <div className="home-subtitle-all">
                <div className="home-subtitle-all-text">
                  {t('pages.home.blocks.members.see')}
                </div>
                <img src={caretRight} alt="Caret right icon" />
              </div>
            </Link>
      ...
      {getBlockRendering(HOMEPAGE_BLOCK.JOURNAL_INDEXATION)?.render && (
        <>
          <h2 className="home-subtitle">
            {t('pages.home.blocks.indexation.subtitle')}
          </h2>
      ...
      {getBlockRendering(HOMEPAGE_BLOCK.LATEST_ACCEPTED_ARTICLES_CAROUSEL)
        ...
        <>
          ...
          <Swiper
            ...
            slidesPerGroup={3}
            cards={
              acceptedArticles?.data.filter(article => article?.title) ?? []
            }
          />
        </>
      )}
