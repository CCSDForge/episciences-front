...
import NewsSidebar, {
  INewsYearSelection,
} from '../../components/Sidebars/NewsSidebar/NewsSidebar';
...
export default function News(): JSX.Element {
  ...
  const { data: news, isFetching } = useFetchNewsQuery(
    {
      rvcode: rvcode!,
      page: currentPage,
  ...
  const renderMobileSelectedYears = (): string =>
  ...
  return (
    <main className="news">
      <Helmet>
        <title>
          {t('pages.news.title')} | {journalName ?? ''}
        </title>
      </Helmet>
      ...
      <div className="news-filtersMobile">
        ...
        >
          <img
            className="news-filtersMobile-tile-icon"
            src={filter}
            alt="List icon"
          />
            ...
            >
              {news?.data.map((singleNews, index) => (
                <NewsCard
                  ...
                  mode={mode}
                  fullCard={
                    mode === RENDERING_MODE.TILE && fullNewsIndex === index
                  }
                  blurCard={
