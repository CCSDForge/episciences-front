...
import {
  ...
  useFetchVolumeQuery,
} from '../../../store/features/volume/volume.query';
...
export default function VolumeDetails(): JSX.Element {
  ...
  const [showFullMobileDescription, setShowFullMobileDescription] =
    useState(false);
  const [openedRelatedVolumesMobileModal, setOpenedRelatedVolumesMobileModal] =
    useState(false);
  ...
  const {
    ...
    { rvcode: rvcode!, vid: id!, language: language },
    { skip: !id || !rvcode }
  );
  const { data: relatedVolumes, isFetching: isFetchingRelatedVolumes } =
    useFetchVolumesQuery(
      {
        rvcode: rvcode!,
        language: language,
  ...
  useEffect(() => {
    if (volume && volume.articles.length > 0) {
      ...
      const paperIds = volume.articles.map(article =>
        article['@id'].replace(/\D/g, '')
      );
  ...
  const fetchArticles = async (paperIds: string[]): Promise<void> => {
    const articlesRequests: Promise<FetchedArticle>[] = paperIds.map(
      async docid => {
        const article: RawArticle = await (
          await fetch(
            `${import.meta.env.VITE_API_ROOT_ENDPOINT}/papers/${docid}`
          )
        ).json();
  ...
  const renderVolumeMobileRelatedVolumes = (): JSX.Element | null => {
    ...
    const caretIcon = (
      <img
        ...
        src={openedRelatedVolumesMobileModal ? caretUp : caretDown}
      />
    );
          ...
          <div className="volumeDetails-content-results-content-description">
            ...
            >
              {showFullMobileDescription
                ...
                : t('common.seeMore')}
              {showFullMobileDescription ? (
                <img src={caretUpLight} />
              ) : (
                <img src={caretDownLight} />
              )}
            </div>
          </div>
        );
    ...
    <main className="volumeDetails">
      ...
      {openedRelatedVolumesMobileModal && (
        <VolumeDetailsMobileModal
          ...
          t={t}
          volume={volume}
          relatedVolumes={relatedVolumes?.data ?? []}
          onSelectRelatedVolumeCallback={(volumeId: number): void =>
            navigate(`${PATHS.volumes}/${volumeId}`)
          }
          onCloseCallback={(): void =>
            setOpenedRelatedVolumesMobileModal(false)
          }
        />
      )}
                ...
                {volume?.types &&
                  ...
                  volume.settingsProceeding.length && (
                    <div className="volumeDetails-content-results-content-proceedingSettings">
                      ...
                      {renderProceedingDOI() && (
                        <Link
                          to={`${import.meta.env.VITE_DOI_HOMEPAGE}/${renderProceedingDOI()}`}
