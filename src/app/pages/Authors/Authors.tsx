...
export default function Authors(): JSX.Element {
  ...
  const { data: authors, isFetching } = useFetchAuthorsQuery(
    {
      rvcode: rvcode!,
      page: currentPage,
  ...
  const getAuthorsCount = (): JSX.Element | null => {
    if (authors) {
      if (authors.totalItems > 1) {
        if (search)
          return (
            <div className="authors-count">
              {authors.totalItems} {t('common.authorsFor')} "{search}"
            </div>
          );
        if (activeLetter)
          return (
            <div className="authors-count">
              {authors.totalItems} {t('common.authorsFor')} "
              {activeLetter === 'others'
                ? t('pages.authors.others')
                : activeLetter}
              "
            </div>
          );
        ...
        return (
          <div className="authors-count">
            {authors.totalItems} {t('common.authors')}
          </div>
        );
      }
      ...
      if (search)
        return (
          <div className="authors-count">
            {authors.totalItems} {t('common.authorFor')} "{search}"
          </div>
        );
      if (activeLetter)
        return (
          <div className="authors-count">
            {authors.totalItems} {t('common.authorFor')} "
            {activeLetter === 'others'
              ? t('pages.authors.others')
              : activeLetter}
            "
      ...
      return (
        <div className="authors-count">
          {authors.totalItems} {t('common.author')}
        </div>
      );
    }
  ...
  return (
    <main className="authors">
      <Helmet>
        <title>
          {t('pages.authors.title')} | {journalName ?? ''}
        </title>
      </Helmet>
