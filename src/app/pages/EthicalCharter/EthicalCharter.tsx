...
import EthicalCharterSidebar, { IEthicalCharterHeader } from '../../components/Sidebars/EthicalCharterSidebar/EthicalCharterSidebar.tsx';
...
export default function EthicalCharter(): JSX.Element {
  ...
  const { data: ethicalCharterPage, isFetching } = useFetchEthicalCharterPageQuery(rvcode!, { skip: !rvcode });

  const parseContentSections = (toBeParsed: string | undefined): IEthicalCharterSection[] => {
  ...
  return (
    <main className='ethicalCharter'>
      <Helmet>
        <title>{t('pages.ethicalCharter.title')} | {journalName ?? ''}</title>
      </Helmet>
      ...
      <h1 className='ethicalCharter-title'>{t('pages.ethicalCharter.title')}</h1>
      ...
      {isFetching ? (
        ...
        <div className='ethicalCharter-content'>
          <EthicalCharterSidebar headers={sidebarHeaders} toggleHeaderCallback={toggleSidebarHeader} />
