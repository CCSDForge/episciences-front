...
export default function ArticleDetails(): JSX.Element {
  ...
  const {
    ...
    { paperid: id!, rvcode: rvcode },
    { skip: !id || !rvcode }
  );
  const { data: relatedVolume, isFetching: isFetchingVolume } =
    useFetchVolumeQuery(
      {
        rvcode: rvcode!,
        vid: article && article.volumeId ? article.volumeId.toString() : '',
        language: language,
      },
      { skip: !article || !article?.volumeId || !rvcode }
    );
  const { data: metadataCSL, isFetching: isFetchingMetadataCSL } =
    useFetchArticleMetadataQuery(
      { rvcode: rvcode!, paperid: id!, type: METADATA_TYPE.CSL },
      { skip: !id || !rvcode }
    );
  const { data: metadataBibTeX, isFetching: isFetchingMetadataBibTeX } =
    useFetchArticleMetadataQuery(
      { rvcode: rvcode!, paperid: id!, type: METADATA_TYPE.BIBTEX },
      { skip: !id || !rvcode }
    );
  ...
  useEffect(() => {
    if (article && !authors.length && !institutions.length) {
      ...
      article.authors.forEach(author => {
        const enhancedAuthor: EnhancedArticleAuthor = {
          ...author,
          institutionsKeys: [],
        };
        ...
        author.institutions?.forEach(institution => {
          ...
          const institutionIndex =
            Array.from(allInstitutionsSet).indexOf(institution);
  ...
  const renderArticleAuthors = (isMobile: boolean): JSX.Element => {
    ...
    const formattedAuthors = authors.map((author, index) => {
      ...
      const authorInstitutions = author.institutionsKeys.map((key, i) => (
        <sup
          key={i}
          className="articleDetails-content-article-authors-institution-key"
        >
          {' '}
          ({key + 1})
        </sup>
      ...
      return (
        <span
          key={index}
          className="articleDetails-content-article-authors-author"
        >
          {authorName}
          {authorInstitutions}
  ...
  const renderSection = (
    ...
    const isOpenedSection = openedSections.find(
    ...
    return (
      <div className="articleDetails-content-article-section">
        ...
        >
          <div className="articleDetails-content-article-section-title-text">
            {sectionTitle}
          </div>
          {isOpenedSection ? (
            <img
  ...
  const [selectedKeywordsLanguage, setSelectedKeywordsLanguage] =
    useState<AvailableLanguage>(language as AvailableLanguage);
  ...
  const getAvailableKeywordsLanguages = (): AvailableLanguage[] => {
    ...
    return Object.keys(article.keywords).filter(
      key => !key.match(/^\d+$/)
  ...
  useEffect(() => {
    if (
      ...
      !availableKeywordsLanguages.includes(selectedKeywordsLanguage)
    ) {
  ...
  const getKeywords = (): string[] => {
    ...
    Object.entries(article.keywords).forEach(([key, values]) => {
      if (key === language) {
        ...
        !availableLanguages.includes(key as AvailableLanguage) &&
        !key.match(/^\d+$/)
      ) {
  ...
  const getLinkedPublicationRow = (
    relatedItem: IArticleRelatedItem
  ): JSX.Element => {
    ...
    if (relatedItem.identifierType === LINKED_PUBLICATION_IDENTIFIER_TYPE.DOI) {
      return (
        <div className="articleDetails-content-article-section-content-linkedPublications-publication">
          ...
          <Link
            to={`${import.meta.env.VITE_DOI_HOMEPAGE}/${relatedItem.value}`}
    ...
    if (
      relatedItem.identifierType === LINKED_PUBLICATION_IDENTIFIER_TYPE.ARXIV
    ) {
      return (
        <div className="articleDetails-content-article-section-content-linkedPublications-publication">
          {relationship && (
          ...
          <Link
            to={`${import.meta.env.VITE_ARXIV_HOMEPAGE}/abs/${relatedItem.value}`}
    ...
    if (relatedItem.identifierType === LINKED_PUBLICATION_IDENTIFIER_TYPE.HAL) {
      return (
        <div className="articleDetails-content-article-section-content-linkedPublications-publication">
          ...
          <Link
            to={`${import.meta.env.VITE_HAL_HOMEPAGE}/${relatedItem.value}`}
    ...
    if (
      ...
      return (
        <div className="articleDetails-content-article-section-content-linkedPublications-publication">
          ...
          <Link
            to={`${import.meta.env.VITE_ARCHIVE_SOFTWARE_HERITAGE_HOMEPAGE}/${relatedItem.value}`}
            target="_blank"
            rel="noopener noreferrer"
          >
            <img
              className="articleDetails-content-article-section-content-linkedPublications-publication-img"
              src={`${import.meta.env.VITE_ARCHIVE_SOFTWARE_HERITAGE_HOMEPAGE}/badge/${relatedItem.value}`}
          ...
          <iframe
            ...
            src={`${import.meta.env.VITE_ARCHIVE_SOFTWARE_HERITAGE_HOMEPAGE}/browse/embed/${relatedItem.value}`}
          ></iframe>
        </div>
      );
    }
      ...
      <ol className="articleDetails-content-article-section-content-references">
        {article?.references.map((reference, index) => (
          ...
          >
            ...
            {reference.doi && (
              <Link
                to={`${import.meta.env.VITE_DOI_HOMEPAGE}/${reference.doi}`}
              ...
              {cb.citations.map((citation, index) => (
                ...
                >
                    ...
                    {citation.authors
                      .map<ReactNode>((author, index) => (
                        <Fragment key={index}>
                          ...
                          {author.orcid && (
                            <Link
                              to={`${import.meta.env.VITE_ORCID_HOMEPAGE}/${author.orcid}`}
                              title={author.orcid}
                              target="_blank"
                              rel="noopener noreferrer"
                            >
                  ...
                  <p className="articleDetails-content-article-section-content-citedBy-row-citations-citation-reference">
                    ...
                    {t('pages.articleDetails.citedBySection.volume')}{' '}
                    {citation.reference.volume}, {citation.reference.year},{' '}
                    {t('pages.articleDetails.citedBySection.page')}{' '}
                  ...
                  <Link
                    ...
                    rel="noopener noreferrer"
                  >
                    {t('pages.articleDetails.citedBySection.doi')} :{' '}
                    {citation.doi}
                  </Link>
                </li>
              ))}
  ...
  const renderMetrics = (): JSX.Element | undefined => {
    if (
      article?.metrics &&
      (article.metrics.views > 0 || article.metrics.downloads > 0)
    ) {
      return (
  ...
  return (
    <main className="articleDetails">
      ...
      {isFetchingArticle ||
      isFetchingVolume ||
      isFetchingMetadataCSL ||
      isFetchingMetadataBibTeX ? (
        <Loader />
      ) : (
        <>
          <ArticleMeta
          ...
          {article?.tag && (
            <div className="articleDetails-tag">
              {t(
                articleTypes.find(tag => tag.value === article.tag)?.labelPath!
              )}
            </div>
          )}
