...
import { useFetchForEditorsPageQuery } from '../../../store/features/forEditors/forEditors.query';
import {
  generateIdFromText,
  unifiedProcessor,
  serializeMarkdown,
...
import ForEditorsSidebar, {
  IForEditorsHeader,
} from '../../components/Sidebars/ForEditorsSidebar/ForEditorsSidebar';
import Breadcrumb from '../../components/Breadcrumb/Breadcrumb';
import Loader from '../../components/Loader/Loader';
import './ForEditors.scss';

interface IForEditorsSection {
  id: string;
  value: string;
  opened: boolean;
}
...
export default function ForEditors(): JSX.Element {
  ...
  const [pageSections, setPageSections] = useState<IForEditorsSection[]>([]);
  const [sidebarHeaders, setSidebarHeaders] = useState<IForEditorsHeader[]>([]);

  const { data: forEditorsPage, isFetching } = useFetchForEditorsPageQuery(
    rvcode!,
    { skip: !rvcode }
  );
  ...
  const parseContentSections = (
    ...
    const sections: IForEditorsSection[] = [];
    let currentSection: IForEditorsSection | null = null;
  ...
  const parseSidebarHeaders = (
    ...
    for (const node of tree.children) {
      if (node.type === 'heading' && (node.depth === 2 || node.depth === 3)) {
        ...
        if (titleText) {
          const header: IForEditorsHeader = {
            id: generateIdFromText(titleText),
            value: titleText,
            opened: true,
            children: [],
  ...
  useEffect(() => {
    const content = getLocalizedContent(forEditorsPage?.content, language);
  ...
  }, [forEditorsPage, language]);

  return (
    <main className="forEditors">
      <Helmet>
        <title>
          {t('pages.forEditors.title')} | {journalName ?? ''}
        </title>
      </Helmet>
      ...
      <Breadcrumb
        ...
        crumbLabel={t('pages.forEditors.title')}
      />

      <h1 className="forEditors-title">{t('pages.forEditors.title')}</h1>

      {isFetching ? (
        <Loader />
      ) : !forEditorsPage ? (
        <div className="forEditors-content">
          <p>{t('pages.forEditors.description')}</p>
        </div>
        ...
        <div className="forEditors-content">
          ...
          />
          <div className="forEditors-content-body">
            {pageSections.map(section => (
              <div
                key={section.id}
                className={`forEditors-content-body-section ${!section.opened && 'forEditors-content-body-section-hidden'}`}
              >
                <MarkdownRenderer
                  urlTransform={uri =>
                    uri.includes('/public/')
                  ...
                  components={{
                    a: ({ ...props }) => (
                      <Link
                        to={props.href!}
                        target="_blank"
                        className="forEditors-content-body-section-link"
                      >
                    ...
                    h2: ({ ...props }) => {
                      ...
                      return (
                        <div
                          className="forEditors-content-body-section-subtitle"
                          onClick={(): void => toggleSectionHeader(id!)}
                        >
                          <h2
                            id={id}
                            className="forEditors-content-body-section-subtitle-text"
                            {...props}
                          />
                          {pageSections.find(
                            ...
                            <img
                              className="forEditors-content-body-section-subtitle-caret"
                              src={caretUp}
                              alt="Caret up icon"
                            />
                          ) : (
