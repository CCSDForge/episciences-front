...
export const volumeApi = createApi({
  ...
  endpoints: build => ({
    ...
    fetchVolume: build.query<
      IVolume,
      { rvcode: string; vid: string; language: AvailableLanguage }
    >({
      query: ({ vid, rvcode }: { vid: string; rvcode: string }) =>
        `volumes/${vid}?rvcode=${rvcode}`,
      transformResponse(
        baseQueryReturnValue: RawVolume,
        _,
        { rvcode, language }
      ) {
