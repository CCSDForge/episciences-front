...
export const newsApi = createApi({
  ...
  tagTypes: ['News'],
  endpoints: (build) => ({
    fetchNews: build.query<{ data: INews[], totalItems: number }, { rvcode: string, page: number, year?: number }>({
      query: ({ rvcode, page, year } :{ rvcode: string, page: number, year?: number }) => year ? `news?page=${page}&rvcode=${rvcode}&year=${year}` : `news?page=${page}&rvcode=${rvcode}`,
