...
export const sectionApi = createApi({
  ...
  endpoints: build => ({
    ...
    >({
      ...
      transformResponse(
        baseQueryReturnValue: PaginatedResponseWithCount<RawSection>,
        _,
        { rvcode }
      ) {
        const articlesCount =
          baseQueryReturnValue['hydra:totalPublishedArticles'];
    ...
    fetchSection: build.query<ISection, { sid: string; rvcode: string }>({
      query: ({ sid, rvcode }: { sid: string; rvcode: string }) =>
        `sections/${sid}?rvcode=${rvcode}`,
